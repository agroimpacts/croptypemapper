...
class Double_branch_stacked_biLSTM(torch.nn.Module):
    def __init__(self, input_dims=(4, 11), hidden_dims=(64, 64), n_classes=4, n_layers=(2, 2), 
                 dropout_rate=(0.35, 0.45), bidirectional=True, use_layernorm=True, 
        ...
        self.s1_lstm = nn.LSTM(input_size=s1_in_dim, hidden_size=s1_hidden_dim, num_layers=s1_n_layers, bias=False, 
                               batch_first=True, dropout=s1_dropout_rate, bidirectional=self.bidirectional)
        
        self.s2_lstm = nn.LSTM(input_size=s2_in_dim, hidden_size=s2_hidden_dim, num_layers=s2_n_layers, bias=False, 
                               batch_first=True, dropout=s2_dropout_rate, bidirectional=self.bidirectional)
        ...
        fused_dense_input_dim = ((s1_hidden_dim + s2_hidden_dim) if self.use_attention else 
                                  (s1_hidden_dim * s1_n_layers) + (s2_hidden_dim * s2_n_layers))
        ...
        if self.use_batchnorm:
            self.s1_bn = nn.BatchNorm1d(s1_hidden_dim)
            self.s2_bn = nn.BatchNorm1d(s2_hidden_dim)
    ...
    def _logits(self, s1, s2):
        ...
        #s1_outputs & s2_outputs: [B, Seq_length, direction x hidden_dim]
        s1_outputs, s1_last_state_list = self.s1_lstm.forward(s1)
        s2_outputs, s2_last_state_list = self.s2_lstm.forward(s2)
        
        #s1_h & s1_c & s2_h & s2_c: [(direction x num_layers), B, hidden_dim] 
        ...
        return fused_logits, s1_pts, s2_pts
    
    
    def forward(self, s1, s2):
        
        fused_logits, s1_pts, s2_pts = self._logits(s1, s2)
